...
export default function OrderDetail() {
  ...
  const commCols = isEditable
    ? (isManager ? '40px 1fr 80px 80px 50px 70px' : '40px 1fr 80px 80px 50px')
    : '40px 1fr 80px 80px'
      ...
      <div key={item.id}
        ...
        <div className="order-cell order-cell-qty">
          {isComm && isEditable ? (
            ...
            <input className="input order-qty-input" type="number" step="any" min="0"
        ...
        {/* Edit/delete — non-commissary managers */}
        {isEditable && !isComm && (
              ...
              <div className="form-group" style={{ margin: 0 }}>
                ...
                <input className="input" type="date" style={{ width: '180px' }}
                  defaultValue={order.expected_date
                    ...
                    : ''}
                  onBlur={async e => {
                    const current = order.expected_date
                      ? new Date(order.expected_date).toISOString().split('T')[0] : ''
