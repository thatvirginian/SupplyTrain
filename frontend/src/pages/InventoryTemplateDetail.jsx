...
function InlineAddButton({ sectionId, insertAfterIdx, addingTo, setAddingTo, onAdd }) {
  ...
  const close = () => { setAddingTo(null); setSearch(''); setResults([]) }

  if (!isOpen) {
    return (
      <div className="inline-add-row">
        <button className="inline-add-btn"
          onClick={() => { setAddingTo({ sectionId, insertAfterIdx }); setSearch(''); setResults([]) }}
          title="Add product here">+</button>
      </div>
    )
  }
...
export default function InventoryTemplateDetail() {
  const { id }   = useParams()
  ...
  const fetchTemplate = useCallback(async () => {
    setLoading(true)
    try {
      const res = await axios.get(`/api/inventory-templates/${id}`)
      setTemplate(res.data)
      setSections(res.data.sections || [])
      setEditForm({ name: res.data.name, notes: res.data.notes || '' })
      setOrderDirty(false)
    } catch (e) { setError('Template not found.') }
  ...
  const handleSaveTemplate = async () => {
    setSaving(true)
    try {
      await axios.put(`/api/inventory-templates/${id}`, editForm)
      setEditing(false)
      setSaved(true)
  ...
  const handleToggleActive = async () => {
    try {
      await axios.post(`/api/inventory-templates/${id}/toggle-active`)
      fetchTemplate()
    } catch (e) { console.error(e) }
  }
  ...
  const handleSaveOrder = async () => {
    setSavingOrder(true)
    try {
      await axios.post(`/api/inventory-templates/${id}/sort`, {
  ...
  const handleAddSection = async () => {
    ...
    setAddingSec(true)
    try {
      await axios.post('/api/inventory-sections', {
  ...
  const handleSaveSection = async (sectionId) => {
    try {
      await axios.put(`/api/inventory-sections/${sectionId}`, { name: editSectionName })
  ...
  const handleDeleteSection = async (sectionId) => {
    ...
    try {
      await axios.delete(`/api/inventory-sections/${sectionId}`)
      fetchTemplate()
    } catch (e) { console.error(e) }
  }
  ...
  const handleAddProduct = async (sectionId, product, insertAfterIdx) => {
    try {
      await axios.post(`/api/inventory-sections/${sectionId}/items`, {
  ...
  const handleRemoveProduct = async (itemId) => {
    try {
      await axios.delete(`/api/inventory-template-items/${itemId}`)
      fetchTemplate()
    } catch (e) { console.error(e) }
  }
  ...
  const handleAddCountUnit = async (itemId) => {
    ...
    try {
      await axios.post(`/api/inventory-template-items/${itemId}/units`, {
  ...
  const handleRemoveCountUnit = async (unitRowId) => {
    try {
      await axios.delete(`/api/inventory-template-item-units/${unitRowId}`)
      fetchTemplate()
    } catch (e) { console.error(e) }
  }
  ...
  const handleToggleLocation = async (locationId, isAssigned) => {
    try {
      if (isAssigned) {
        await axios.delete(`/api/inventory-templates/${id}/locations/${locationId}`)
      } else {
        await axios.post(`/api/inventory-templates/${id}/locations`, { location_id: locationId })
      }
      fetchTemplate()
    } catch (e) { console.error(e) }
  }
    ...
    <div className="page">
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <button className="btn btn-ghost" onClick={() => navigate('/inventory-templates')}>← Back</button>
          <div>
          ...
          {sections.map((section, sectionIdx) => {
            ...
            return (
              <div key={section.id}
                ...
                onDragOver={e => { if (dragItem.current?.type === 'item') return; e.preventDefault() }}
                onDrop={e => {
                ...
                {!isCollapsed && (
                  <div>
                    {(!section.items || section.items.length === 0) && (
                      <div className="comm-empty-drop-zone"
                    ...
                    <InlineAddButton sectionId={section.id} insertAfterIdx={-1}
                      addingTo={addingTo} setAddingTo={setAddingTo} onAdd={handleAddProduct} />
                        ...
                        <div key={item.id}>
                          <div className="section-item"
                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', flex: 1 }}>
                              <span className="drag-handle" style={{ marginTop: '2px' }}
                                draggable
                          ...
                          <InlineAddButton sectionId={section.id} insertAfterIdx={itemIdx}
                            addingTo={addingTo} setAddingTo={setAddingTo} onAdd={handleAddProduct} />
                        </div>
                      ))}
                    </div>
